...
int main(int argc, char** argv)
{
    ...
    for (size_t query_id = 0; query_id < query_num; query_id++) {
        ...
        size_t start_addr_in_DRAM_per_query = query_id * (size_header + size_cell_IDs + size_LUTs) * 64 / sizeof(int);
        size_t start_addr_in_DRAM_cell_ID = start_addr_in_DRAM_per_query + size_header * 64 / sizeof(int);
        size_t start_addr_in_DRAM_LUT = start_addr_in_DRAM_per_query + (size_header + size_cell_IDs) * 64 / sizeof(int);
        ...
        // std::cout << " 6\n";
            for (size_t row_id = 0; row_id < 256; row_id++) {
                for (size_t m = 0; m < M; m++) {
                    ...
                    in_DRAM[start_addr_in_DRAM_LUT + nprobe_id * 256 * M + row_id * M + m] = *((int*) (&dist));
