...
//     }
    ...
    ap_uint<32> raw_vec_ID = source_hw_results[0].range(31, 0);
    ap_uint<32> dist_ap = source_hw_results[0].range(63, 32);

    int vec_ID = *((int*) (&raw_vec_ID));
    float dist = *((float*) (&dist_ap));
    std::cout << "      vec_ID = " << vec_ID <<  "    distance = " << dist << std::endl;
