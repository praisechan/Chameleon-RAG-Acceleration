...
// #pragma HLS array_partition variable=L1_dist complete
                    ...
                    float result = 0;
                    for (int j = 0; j < D / M; j++) {
#pragma HLS UNROLL factor=2 // let pipeline depth in the outer loop to infer the unroll factor
                        float L1_dist = sub_residual_vector[j] - sub_product_quantizer[k][j];
                        float L2_dist = L1_dist * L1_dist;
                        result += L2_dist;
                    }
                
                s_partial_distance_LUT.write(result);
            }
        }
    }
}
